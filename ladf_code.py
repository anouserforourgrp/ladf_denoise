...
def proces_scene(root_path, td_param, sd_param):
    ...
    for subdir, _, files in os.walk(root_path):
        ...
        de_files = [file for file in files if file.endswith('tsdraw_background_removed.npy')]
        cali_files = [file for file in files if file.endswith('cali.npy')]

        if not de_files or not cali_files:
            continue
        ...
        for de_file in de_files:
            prefix = '_'.join(de_file.split('_')[:2])
            corresponding_cali_file = None
            for cali_file in cali_files:
                if cali_file.startswith(prefix):
                    corresponding_cali_file = cali_file
                    break

            if corresponding_cali_file is not None:
                de_file_path = os.path.join(subdir, de_file)
                cali_file_path = os.path.join(subdir, corresponding_cali_file)

                de_array = np.load(de_file_path)
                cali_array = np.load(cali_file_path)
                de_array_tensor = torch.from_numpy(de_array).to(device)
                cali_array_tensor = torch.from_numpy(cali_array).to(device)

                if de_file == '000_00_tsdraw_background_removed.npy':

                    LSD_reconstructed, label_l, RSD_reconstructed, label_r = sd_denoise_4dir_adp_test(de_array_tensor[..., 1],
                                                                                                      de_array_tensor[..., 2],sd_param)
                    ...
                    LSD_reconstructed, label_l, RSD_reconstructed, label_r = sd_denoise_4dir_adp_test(de_array_tensor[..., 1],
                                                                                                          de_array_tensor[..., 2],sd_param)
                    sd_time.append(time.time() - t0)
                if de_file == '000_00_tsdraw_background_removed.npy':
                    TD_reconstructed, label_td = td_denoise(de_array_tensor[..., 0],td_param)
                else:
                    t1 = time.time()
                    TD_reconstructed, label_td = td_new(last_lsd, last_rsd, LSD_reconstructed, RSD_reconstructed,
                                                        de_array_tensor[..., 0], td_param)
                    td_time.append(time.time() - t1)
                    last_lsd = LSD_reconstructed
                    last_rsd = RSD_reconstructed

                TP, TN, FP, FN = calculate_metrics(cali_array_tensor[0, ...], label_td)
                TP_td += TP
                TN_td += TN
                FP_td += FP
                FN_td += FN

                TP, TN, FP, FN = calculate_metrics(cali_array_tensor[1, ...], label_l)
                TP_sdl += TP
                TN_sdl += TN
                FP_sdl += FP
                FN_sdl += FN

                TP, TN, FP, FN = calculate_metrics(cali_array_tensor[2, ...], label_r)
                TP_sdr += TP
                TN_sdr += TN
                FP_sdr += FP
                FN_sdr += FN
            else:
                print(f"file corresponding to {de_file} not found.")

    tdsr = TP_td / (TP_td + FN_td)
    tdnr = TN_td / (TN_td + FP_td)
    tdda = (tdsr + tdnr) / 2
    tdsnr = TP_td / FP_td
    sdsr = (TP_sdl + TP_sdr) / (TP_sdl + FN_sdl + TP_sdr + FN_sdr)
    sdnr = (TN_sdl + TN_sdr) / (TN_sdl + FP_sdl + TN_sdr + FP_sdr)
    sdda = ((TP_sdl + TP_sdr) / (TP_sdl + FN_sdl + TP_sdr + FN_sdr) + (TN_sdl + TN_sdr) / (TN_sdl + FP_sdl + TN_sdr + FP_sdr)) / 2
    sdsnr = (TP_sdl + TP_sdr) / (FP_sdl + FP_sdr)

    print(
        f"TD:sr: {tdsr}, nr: {tdnr}, da: {tdda},snr: {tdsnr}", )

    print(
        f"SD:sr: {sdsr}, nr: {sdnr}, da: {sdda},snr: {sdsnr }", )

if __name__ == '__main__':
    root_path="THE PATH TO DATASET"
    td_param = {
        'var_fil_ksize': 3,
        'var_th': 1,
        'adapt_th_min': 3,
    ...
    proces_scene(root_path, td_param, sd_param)
